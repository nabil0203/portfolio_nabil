        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-8 lg:gap-10 max-w-7xl mx-auto">
          {Object.entries(skillGroups).map(([category, skills], groupIndex) => (
            <MotionDiv key={category} delay={groupIndex * 0.1}>
              <motion.div
                className="group relative flex flex-col h-full bg-surface/20 backdrop-blur-xl p-8 rounded-3xl border border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.3)] overflow-hidden transition-all duration-300"
                whileHover={{ y: -6 }}
              >
                ...
                <div className="relative z-10 w-full">
                  <div className="flex items-center gap-5 mb-5">
                    <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-accent to-accent-secondary flex items-center justify-center p-[1px] shadow-[0_0_20px_rgba(59,130,246,0.3)] group-hover:shadow-[0_0_30px_rgba(59,130,246,0.5)] transition-shadow duration-500">
                      <div className="w-full h-full bg-surface/90 rounded-2xl flex items-center justify-center relative overflow-hidden">
                        <div className="w-3.5 h-3.5 bg-accent-glow rounded-full shadow-[0_0_20px_rgba(6,184,212,1)]" />
                      </div>
                    </div>
                    <h3 className="text-2xl font-bold tracking-wide text-transparent bg-clip-text bg-gradient-to-r from-white to-white/80 group-hover:to-white transition-colors duration-300">
                      {category}
                    </h3>
                  </div>

                  <div className="w-full h-[1px] bg-gradient-to-r from-accent/40 via-accent-secondary/20 to-transparent mb-8" />
                      ...
                      return (
                        ...
                        >
                          <SkillComponent
                            ...
                            className={`group/pill relative overflow-hidden inline-flex items-center px-4 py-2.5 bg-white/[0.03] backdrop-blur-md rounded-xl border border-white/10 text-sm font-semibold text-slate-300 transition-all duration-300 shadow-sm ${skill.url ? "hover:bg-white/[0.08] hover:border-accent/60 hover:text-white hover:shadow-[0_0_20px_rgba(59,130,246,0.25)] cursor-pointer" : "cursor-default hover:text-white hover:border-white/20 hover:bg-white/[0.06]"
                              }`}
                          ...
                          >
                            ...
                            {skill.url && (
                              <svg className="relative z-10 w-3.5 h-3.5 ml-2 opacity-50 group-hover/pill:opacity-100 group-hover/pill:text-accent-glow transition-all duration-300 group-hover/pill:translate-x-0.5 group-hover/pill:-translate-y-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
